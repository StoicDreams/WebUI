...
pub const INVALID: i32 = 1_073_741_824;

pub fn roles_contains_role(roles: i32, role: i32) -> bool {
    roles & role == role
}
...
///
/// Invalid level is 31.
/// Example: role_from_level(31) returns INVALID
pub fn role_from_level(level: u8) -> i32 {
    if level > 31 {
        return 0;
    }
    if level == 0 {
...
mod tests {
    ...
    fn test_role_values() {
        ...
        assert_eq!(role_from_level(29), ADMIN);
        assert_eq!(role_from_level(30), SITE_ADMIN);
        assert_eq!(role_from_level(31), INVALID);
    }
    ...
    fn test_roles() {
        assert_eq!(PUBLIC, 0);
        assert_eq!(USER, 1);
        assert_eq!(ADMIN, 268_435_456);
        assert_eq!(SITE_ADMIN, 536_870_912);
